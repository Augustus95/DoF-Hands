...
public:
	...
	void setWireframe(bool a) { isWireFrame = a; }
	void setSize(float size) { this->size = size; }
	float getSize() { return size; }
	void ObjWrite(const char*filepath);
	Vector3 getCenter() {
		Vector3 vec3 = { 0,0,0 };
		for (auto iter=vertexList.begin(); iter != vertexList.end(); iter++) {
			vec3.x += iter->x; vec3.y += iter->y; vec3.z += iter->z;
		}
		vec3.x /= vertexList.size() * 3; vec3.y /= vertexList.size() * 3; vec3.z /= vertexList.size() * 3;
		return vec3;
	}
};
...
void OBJModel::readString(const char *str)
{
	...
	else if (_strcmpi(cmd, "f") == 0)
	{
		...
		if (count <= 6) {
			...
			for (int j = 0; j < 4; j++)
			{
				q.UV_Pnt[j] = q.Normal[j] = -1;
				sscanf_s(str + token[j], "%d/%d/%d", &q.Vertex[j], &q.UV_Pnt[j], &q.Normal[j]);
			}

			q.mat = currentMat;
#ifdef DEBUG
			printf("Face <%d-%d-%d-%d>\n", q.Vertex[0], q.Vertex[1], q.Vertex[2],q.Vertex[3]);
			if (q.mat != NULL)
				printf("Material : %s\n", q.mat->name);
#endif
...
void OBJModel::Draw(int mode)
{
	...
	for (std::list<Quad>::const_iterator it = quadList.begin(); it != quadList.end(); it++)
	{
		if (it->mat != NULL)
		{
			float Ka[4] = { it->mat->Ka[0], it->mat->Ka[1], it->mat->Ka[2], it->mat->Tr };
...
void OBJModel::ObjWrite(const char*filename) {
	FILE *fp;
	fopen_s(&fp, filename, "r");
	if (fp == NULL)
	{
